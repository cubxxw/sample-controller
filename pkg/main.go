...
func main() {
	...
	controller := NewBlogController(kubeClient, exampleClient,
		kubeInformerFactory.Apps().V1().Deployments(),
		exampleInformerFactory.Samplecontroller().V1alpha1().Foos())
 
	blogController := NewBlogController(kubeClient, blogClient,
		kubeInformerFactory.Apps().V1().Deployments(),
		blogInformerFactory.Samplecontroller().V1alpha1().Blogs())
 
	// notice that there is no need to run Start methods in a separate goroutine. (i.e. go kubeInformerFactory.Start(stopCh)
	// Start method is non-blocking and runs all registered informers in a dedicated goroutine.
	kubeInformerFactory.Start(stopCh)
	exampleInformerFactory.Start(stopCh)
	blogInformerFactory.Start(stopCh)
	...
	if err = blogController.Run(2, stopCh); err != nil {
		klog.Fatalf("Error running controller: %s", err.Error())
	}
}
